    ...
    async def _setup_database(self, conn: aiosqlite.Connection) -> None:
        ...
        await conn.execute('''
            CREATE TABLE IF NOT EXISTS forum_post_monitor_configs (
                ...
                summon_panel_channel_id TEXT,
                summon_allowed_role_ids TEXT,
                summon_panel_ping_enabled BOOLEAN DEFAULT FALSE,
                manual_notify_channel_id TEXT,
                manual_notify_role_ids TEXT,
                manual_notify_template TEXT,
                created_at TEXT NOT NULL,
        ...
        try:
            ...
            pass
        try:
            await conn.execute("ALTER TABLE forum_post_monitor_configs ADD COLUMN notification_mode TEXT DEFAULT 'instant'")
            logger.info("数据库迁移: 已为 forum_post_monitor_configs 添加列 notification_mode")
        except Exception:
            pass
        try:
            await conn.execute("ALTER TABLE forum_post_monitor_configs ADD COLUMN summon_panel_channel_id TEXT")
            logger.info("数据库迁移: 已为 forum_post_monitor_configs 添加列 summon_panel_channel_id")
        except Exception:
            pass
        try:
            await conn.execute("ALTER TABLE forum_post_monitor_configs ADD COLUMN summon_allowed_role_ids TEXT")
            logger.info("数据库迁移: 已为 forum_post_monitor_configs 添加列 summon_allowed_role_ids")
        except Exception:
            pass
        try:
            await conn.execute("ALTER TABLE forum_post_monitor_configs ADD COLUMN summon_panel_ping_enabled BOOLEAN DEFAULT FALSE")
            logger.info("数据库迁移: 已为 forum_post_monitor_configs 添加列 summon_panel_ping_enabled")
        except Exception:
            pass
        try:
            await conn.execute("ALTER TABLE forum_post_monitor_configs ADD COLUMN manual_notify_channel_id TEXT")
            logger.info("数据库迁移: 已为 forum_post_monitor_configs 添加列 manual_notify_channel_id")
        except Exception:
            pass
        try:
            await conn.execute("ALTER TABLE forum_post_monitor_configs ADD COLUMN manual_notify_role_ids TEXT")
            logger.info("数据库迁移: 已为 forum_post_monitor_configs 添加列 manual_notify_role_ids")
        except Exception:
            pass
        try:
            await conn.execute("ALTER TABLE forum_post_monitor_configs ADD COLUMN manual_notify_template TEXT")
            logger.info("数据库迁移: 已为 forum_post_monitor_configs 添加列 manual_notify_template")
        except Exception:
            pass
        ...
        # --- 论坛新帖手动通知请求表（用于按钮召唤通知与去重） ---
        await conn.execute('''
            CREATE TABLE IF NOT EXISTS forum_post_notification_requests (
                thread_id TEXT PRIMARY KEY,
                guild_id TEXT NOT NULL,
                forum_channel_id TEXT NOT NULL,
        ...
        ''')
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_fpnr_guild_status ON forum_post_notification_requests (guild_id, status)")
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_fpnr_panel_message ON forum_post_notification_requests (panel_message_id)")
