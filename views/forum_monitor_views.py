...
async def _parse_message_channel_id(guild: discord.Guild, raw: Optional[str], client: discord.Client) -> Optional[str]:
    channel_id = _extract_message_channel_id(raw)
    if not channel_id:
        return None
    ...
    try:
        ...
        if fetched_guild and getattr(fetched_guild, "id", None) != guild.id:
            return None
...
def _format_channel_id_for_display(guild: discord.Guild, raw: Optional[str]) -> str:
    channel_id = _extract_message_channel_id(raw)
    if not channel_id:
        return "无"
...
def _format_channel_id_for_input(raw: Optional[str]) -> str:
    channel_id = _extract_message_channel_id(raw)
    return f"<#{channel_id}>" if channel_id else ""


def _format_role_ids_for_input(raw: Optional[str]) -> str:
    if not raw or not isinstance(raw, str):
        return ""
    parts = [p.strip() for p in raw.split(",") if p and p.strip()]
    mentions: List[str] = []
...
def _config_bool(v: Any, default: bool = False) -> bool:
...
def _build_flags_for_input(config: Dict[str, Any]) -> str:
    return (
        f"auto={'yes' if _config_bool(config.get('auto_role_enabled')) else 'no'},"
        f"notify={'yes' if _config_bool(config.get('notify_enabled'), default=True) else 'no'},"
        f"mention={'yes' if _config_bool(config.get('mention_role_enabled')) else 'no'},"
        f"cross={'yes' if _config_bool(config.get('cross_post_enabled')) else 'no'},"
        f"append_link={'yes' if _config_bool(config.get('cross_post_append_link'), default=True) else 'no'},"
        f"panel_ping={'yes' if _config_bool(config.get('summon_panel_ping_enabled')) else 'no'}"
    )


def _build_reminder_config_for_input(config: Optional[Dict[str, Any]]) -> str:
    if not config:
        return ""

    mode = str(config.get("notification_mode") or "instant").strip().lower()
    lines: List[str] = []
    if mode == "manual":
        lines.append("mode=manual")
        lines.append(f"panel_channel={_format_channel_id_for_input(config.get('summon_panel_channel_id'))}")
        summon_roles = _format_role_ids_for_input(config.get("summon_allowed_role_ids"))
        if summon_roles:
            lines.append(f"summon_roles={summon_roles}")
        notify_channel = config.get("manual_notify_channel_id") or config.get("cross_post_channel_id")
        lines.append(f"notify_channel={_format_channel_id_for_input(notify_channel)}")
        notify_roles = _format_role_ids_for_input(config.get("manual_notify_role_ids") or config.get("cross_post_role_ids"))
        if notify_roles:
            lines.append(f"notify_roles={notify_roles}")
        template = config.get("manual_notify_template") or config.get("cross_post_template")
        if template:
            lines.append(f"notify_template={str(template).strip()}")
    else:
        channel = _format_channel_id_for_input(config.get("cross_post_channel_id"))
        if channel:
            lines.append(f"channel={channel}")
        roles = _format_role_ids_for_input(config.get("cross_post_role_ids"))
        if roles:
            lines.append(f"roles={roles}")
        template = config.get("cross_post_template")
        if template:
            lines.append(f"template={str(template).strip()}")

    lines.append(f"flags={_build_flags_for_input(config)}")
    return "\n".join(lines)
...
def _parse_flags(raw: Optional[str]) -> Dict[str, bool]:
    """
    解析flags文本（可选），格式示例：
    auto=yes, notify=yes, mention=no, cross=yes, append_link=yes, panel_ping=no
    支持: yes/no/true/false/1/0/y/n/t/f
    未提供时默认: notify=True, append_link=True，其它False
    """
    default = {"auto": False, "notify": True, "mention": False, "cross": False, "append_link": True, "panel_ping": False}
    if not raw or not isinstance(raw, str):
        return default
    ...
    try:
        ...
        for p in parts:
            ...
            k = k.strip().lower()
            v = v.strip()
            if k in ("auto", "notify", "mention", "cross", "append_link", "panel_ping", "summon_ping"):
                if k == "summon_ping":
                    k = "panel_ping"
                flags[k] = to_bool(v)
...
class ForumMonitorConfigModal(ui.Modal, title="配置论坛频道监控"):
    ...
    - cross_post_config: 跨频道提醒/手动召唤综合配置（多行）
      channel=频道ID、<#频道>或 Discord 链接
      roles=角色ID列表或<@&角色>列表（逗号分隔）
      template=模板（支持 {thread_url} {thread_title} {forum_name} {author_mention}）
      mode=manual 时支持 panel_channel/summon_roles/notify_channel/notify_roles/notify_template
    ...
    notify_message = ui.TextInput(label="线程内贴主通知文案（可选）", style=discord.TextStyle.paragraph, required=False, placeholder="默认：欢迎加入讨论！")
    ...
    cross_post_config = ui.TextInput(
        label="提醒/召唤配置（每行一个key=value）",
        style=discord.TextStyle.paragraph,
        required=False,
        placeholder="手动必填: mode, panel_channel, summon_roles, notify_channel, notify_roles, flags"
    )

    def __init__(self, guild: discord.Guild, channel_id: str, existing_config: Optional[Dict[str, Any]] = None):
        super().__init__(timeout=180)
        self.guild = guild
        self.channel_id = channel_id
        if existing_config:
            self.auto_role_id.default = _format_role_ids_for_input(existing_config.get("auto_role_id"))
            self.notify_message.default = str(existing_config.get("notify_message") or "")
            self.mention_role_id.default = _format_role_ids_for_input(existing_config.get("mention_role_id"))
            self.mention_message.default = str(existing_config.get("mention_message") or "")
            self.cross_post_config.default = _build_reminder_config_for_input(existing_config)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        cross_channel_id = await _parse_message_channel_id(self.guild, cross_map.get("channel"), interaction.client)
        ...
        notification_mode_raw = (cross_map.get("mode") or "instant").strip().lower()
        notification_mode = "manual" if notification_mode_raw in ("manual", "手动") else "instant"
        summon_panel_channel_id = await _parse_message_channel_id(
            self.guild,
            cross_map.get("panel_channel") or cross_map.get("panel"),
            interaction.client,
        )
        summon_role_ids = _parse_role_ids_csv(self.guild, cross_map.get("summon_roles")) if cross_map.get("summon_roles") else []
        manual_notify_channel_id = await _parse_message_channel_id(
            self.guild,
            cross_map.get("notify_channel"),
            interaction.client,
        )
        manual_notify_role_ids = _parse_role_ids_csv(self.guild, cross_map.get("notify_roles")) if cross_map.get("notify_roles") else []
        manual_notify_template_raw = cross_map.get("notify_template") or cross_template_raw
        manual_notify_template = _truncate_message(manual_notify_template_raw) if manual_notify_template_raw else None

        if notification_mode == "manual":
            # 兼容旧的跨频道提醒写法：手动模式未单独配置 notify_* 时复用 channel/roles/template。
            manual_notify_channel_id = manual_notify_channel_id or cross_channel_id
            if not manual_notify_role_ids:
                manual_notify_role_ids = list(cross_role_ids)
            manual_notify_template = manual_notify_template or cross_template
        ...
        panel_ping_enabled = flags_dict.get("panel_ping", False)

        # 默认通知文案
        if notify_enabled and not notify_msg:
            notify_msg = "欢迎加入讨论！"

        if notification_mode == "manual" and not summon_panel_channel_id:
            await interaction.followup.send("❌ 手动召唤模式必须配置有效的 panel_channel（频道、子区ID或Discord链接）。", ephemeral=True)
            return
        ...
        try:
            ...
            await cog._upsert_config(
                ...
                summon_panel_channel_id=summon_panel_channel_id,
                summon_allowed_role_ids=",".join(summon_role_ids) if summon_role_ids else None,
                summon_panel_ping_enabled=panel_ping_enabled,
                manual_notify_channel_id=manual_notify_channel_id,
                manual_notify_role_ids=",".join(manual_notify_role_ids) if manual_notify_role_ids else None,
                manual_notify_template=manual_notify_template,
            )
...
class ForumMonitorPanelView(ui.View):
    ...
    async def show_current_config(self, interaction: discord.Interaction):
        ...
        if config:
            ...
            cross_target = _format_channel_id_for_display(interaction.guild, config.get("cross_post_channel_id"))
            ...
            mode = str(config.get("notification_mode") or "instant").strip().lower()
            panel_target = _format_channel_id_for_display(interaction.guild, config.get("summon_panel_channel_id"))
            summon_roles = _format_role_ids_for_display(interaction.guild, config.get("summon_allowed_role_ids"))
            panel_ping_text = b2e(config.get("summon_panel_ping_enabled"))
            manual_channel_id = config.get("manual_notify_channel_id") or config.get("cross_post_channel_id")
            manual_roles_raw = config.get("manual_notify_role_ids") or config.get("cross_post_role_ids")
            manual_notify_target = _format_channel_id_for_display(interaction.guild, manual_channel_id)
            manual_notify_roles = _format_role_ids_for_display(interaction.guild, manual_roles_raw)
            manual_template = (config.get("manual_notify_template") or config.get("cross_post_template") or "默认模板").strip()
            fallback_note = ""
            if mode == "manual" and (
                (not config.get("manual_notify_channel_id") and config.get("cross_post_channel_id"))
                or (not config.get("manual_notify_role_ids") and config.get("cross_post_role_ids"))
                or (not config.get("manual_notify_template") and config.get("cross_post_template"))
            ):
                fallback_note = "\n兼容: 未单独设置 notify_* 的项目会复用 channel/roles/template"
            embed.add_field(
                name="手动召唤通知",
                value=(
                    f"模式: {'手动召唤' if mode == 'manual' else '立即通知'} | 面板频道: {panel_target}\n"
                    f"可召唤身份组: {summon_roles} | 面板提醒: {panel_ping_text}\n"
                    f"通知频道: {manual_notify_target} | 通知身份组: {manual_notify_roles}\n"
                    f"通知模板: {manual_template}{fallback_note}"
                ),
                inline=False,
            )
        role_ids = await cog._list_panel_permission_role_ids(guild_id)
    ...
    async def configure_button(self, interaction: discord.Interaction, button: ui.Button):
        ...
        if not self.selected_channel_id:
            ...
            return
        try:
            existing_config = await cog._get_config(str(interaction.guild.id), self.selected_channel_id)
            modal = ForumMonitorConfigModal(interaction.guild, self.selected_channel_id, existing_config)
    ...
    async def help_button(self, interaction: discord.Interaction, button: ui.Button):
        ...
        guide_embed.add_field(
            ...
            value=(
                "```\n"
                "mode=manual\n"
                "panel_channel=<#审核频道ID或子区链接>\n"
                "summon_roles=<@&可召唤身份组>\n"
                "notify_channel=<#正式通知频道ID或子区链接>\n"
                "notify_roles=<@&被通知身份组1>,<@&被通知身份组2>\n"
                "notify_template={author_mention} 在 {forum_name} 发布了新帖子：[{thread_title}]({thread_url})\n"
                "flags=auto=no,notify=no,mention=no,cross=no,append_link=yes,panel_ping=yes\n"
                "```"
            ),
            inline=False,
        )
        guide_embed.add_field(
            ...
            value=(
                ...
                "- `{operator_mention}`: 点击召唤按钮的人\n"
                "- `{extra_message}`: 追加内容发送时输入的文本"
            ),
            inline=False,
        )
        guide_embed.add_field(
            ...
            value=(
                ...
                "- 未识别变量会保留原文，不会报错\n"
                "- `roles=` 和 `notify_roles=` 可留空，留空则只发文本不@身份组\n"
                "- `channel/panel_channel/notify_channel` 支持频道ID、频道提及或 Discord 子区链接\n"
                "- `panel_ping=yes` 会在审核面板发布时 @ `summon_roles`，提醒他们来点按钮\n"
                "- `mode=manual` 必须配置有效的 `panel_channel=`\n"
                "- 手动模式未写 `notify_channel/notify_roles/notify_template` 时，会兼容复用 `channel/roles/template`"
            ),
            inline=False,
        )
